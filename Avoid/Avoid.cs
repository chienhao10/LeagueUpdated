...
namespace Avoid
{
    public class Avoid
    {
        ...
        {
            // Check for updates
            UpdateChecker.Initialize("LordZEDith/LeagueUpdated/tree/master/Avoid");
        ...
        private static void OnPropertyChange(GameObject sender, GameObjectFloatPropertyChangeEventArgs args)
        {
            ...
            if (key != null)
            {
                    ...
                    args.NewValue == 0)
                {
                    ...
                    if (baseObject != null && ObjectManager.Player.CharData.BaseSkinName == "Nidalee_Spear")
                    {
        ...
        private static void OnDelete(GameObject sender, EventArgs args)
        {
            if (sender.IsValid)
            {
                ...
                foreach (var entry in _avoidableObjects)
                {
                    if (entry.Key.NetworkId != sender.NetworkId) continue;
